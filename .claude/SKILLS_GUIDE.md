...
- **ai-playbook**: Four-layer AI design playbook (Principles, Operating Controls, Security, Launch & Ops). Use when starting any AI feature, choosing LLM vs rules, designing RAG, or needing a decision framework. Router to decision records, launch checks, and security review.
- **ai-decision-record**: 18-question structured questionnaire to document an AI feature before building — problem framing, design choices, eval strategy, rollback plan, risk register. Required input for ai-launch-check. Use at feature kickoff.
- **ai-launch-check**: Pre-flight gate for shipping AI features — 10 questions, 16 artifacts checklist, 8 kill-switch tests. Outputs GO / NO-GO / GO-WITH-EXCEPTIONS. Requires completed Decision Record. Use before any AI feature goes to production.
- **ai-audit**: Orchestrates a 5-phase audit of existing AI features (reverse-engineer Decision Record, security subagent, eval subagent, launch check, remediation plan). Use to audit a shipped AI feature or after an incident.
- **ai-incident-response**: 4-stage AI incident procedure — stabilize, triage, investigate, learn. Covers rollback decisions, kill-switch firing, crime-scene log capture, root-cause investigation, post-mortem template, eval gap identification. Use when a production AI feature is failing.
...
### Frontend (19 skills)
- **a2ui-angular**: A2UI (Agent-to-User Interface) renderer development for Angular 21.x — protocol implementation, component catalog, recursive renderer, action handling, streaming A2UI payloads, and security validation. Reference files: `a2ui-protocol.md`, `a2ui-protocol-advanced.md`, `a2ui-security.md`, `a2ui-component-catalog.md`, `a2ui-component-containers.md`, `a2ui-renderer-patterns.md`, `a2ui-renderer-template.md`, `a2ui-chat-template.md`, `a2ui-renderer-services.md`, `a2ui-client-integration.md` (agUiResource service pattern, demo-only vs production action handler warning, rate limits).
- **streaming-protocols**: Three-layer mental model (Transport/Framing/Message) for all AI streaming. Covers: SSE vs HTTP Streaming clarification (SSE is a subset, not a competitor), bidirectionality guide (half-duplex vs full-duplex, when to pick SSE vs WebSocket), MCP bidirectionality pattern (dual GET+POST), real-world wire formats (OpenAI delta, Anthropic typed events, MCP JSON-RPC, A2UI over SSE/NDJSON), production pitfalls (proxy buffering, compression breaks SSE, `Last-Event-ID` mobile reconnection, `traceparent` propagation), decision guide, NDJSON niche. Load when designing or debugging any streaming in AI systems.
