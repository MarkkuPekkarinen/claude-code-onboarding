...
2. **Add production dependencies** — `uv add "langchain-core>=1.2.0" "langchain-anthropic>=1.3.0" "langchain-openai>=1.1.0" "langgraph>=1.0.0" "langgraph-checkpoint-postgres>=3.0.0" "fastapi>=0.128.0" "uvicorn[standard]" gunicorn pydantic pydantic-settings langsmith prometheus-client structlog httpx asyncpg`
